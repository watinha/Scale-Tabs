...
function Screenshot_window(){
  ...
  tabs.on("ready", function(tab){
    if(tab.title != self.constants.SCALE_TABS_TAB_TITLE){
      ...
      timer.setTimeout(function(){self.load_tab_data(tab);}, 5000);
      timer.setTimeout(function(){self.load_tab_data(tab);}, 10000);
    }
  });
};
...
Screenshot_window.prototype.change_tab = function(tab_title){
  /*
   * Solving the problem of glimpsing a tab before actually going for the correct one.
   */
  for each (var window in windows){
    for each (var tab in window.tabs){
      ...
      if(tab.title == this.constants.SCALE_TABS_TAB_TITLE)
        timer.setTimeout(function(){tab.close();}, 100);
    }
  }
};
